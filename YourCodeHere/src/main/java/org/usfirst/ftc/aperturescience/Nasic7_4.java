...
public class Nasic7_4 extends OpMode {
    ...
    private final double NORMAL_DRIVE_SPEED = 0.65;
    ...
    private void armControl() {
        ...
        if (autoArmUp) {
            ...
            if (armPos < AUTO_ARM_MAX) {
                ...
                arm.setPower(Range.clip(AUTO_ARM_MAX - armPos / 400, 0.3, .75));
            } else {
        ...
        if (autoArmDown) {
            ...
            if (armPos < ARM_TTLIMIT_POS && (Math.abs(ttPos) > TT_CENTER_RANGE || !armExtIn)) {
                ...
                arm.setPower(-Range.clip(armPos / 400, 0.3, 0.75));
            } else {
                autoArmDown = false;
                wristPos = WDOWN;
                arm.setPower(0.0);
    ...
    private final double WDOWN = 0.4;
    private final double WUP = 0.05;
    ...
    private final double ARM_EXT_IN = .75;
    private final double ARM_EXT_OUT = .47;
    ...
    /* blue arm constants */
    private final double BLUE_INIT = 0.05;
    ...
    private final double RTAPE_UP = 0.29;
    ...
    private void tapeControl() {
        ...
        tapeServoPosRight = Range.clip(tapeServoPosRight, RTAPE_UP, RTAPE_UP + .68);

        tapeServoPosLeft = Range.clip(tapeServoPosLeft, 0, 1);
        tapeServoPosRight = Range.clip(tapeServoPosRight, 0, 1);
